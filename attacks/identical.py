...
def make_attack(compute_direction):
  ...
  """
  def attack(grad_honests, f_real, f_decl, defense, gar, factor=-16, negative=False, clip_thresh=None, bucket_size=None, gar_second=None, pivot=None, sigma_filterL2=None, device=None, **kwargs):
    ...
    Args:
      ...
      clip_thresh           Clipping threshold in case of centered clipping aggregation
    ...
    if factor < 0:
      def eval_factor(factor):
        ...
        if gar == "centeredclip":
            #JS: CC is used as aggregation rule
            aggregated_list = list()
            for i in range(len(grad_honests)):
                aggregated_list.append(defense(gradients=(grad_honests + [grad_attack] * f_real), g_prev=grad_honests[i], clip_thresh=clip_thresh,f=f_decl))
            stacked_grads = torch.stack(aggregated_list)
            aggregated = stacked_grads.mean(dim=0).sub(grad_avg)
        elif gar == "cva" and pivot is None:
            #JS: CVA is used as aggregation rule, in P2P setting
            aggregated_list = list()
            for i in range(len(grad_honests)):
                aggregated_list.append(defense(gradients=(grad_honests + [grad_attack] * f_real), honest_index=i, f=f_decl))
            stacked_grads = torch.stack(aggregated_list)
            aggregated = stacked_grads.mean(dim=0).sub(grad_avg)
        else:
            aggregated = defense(gradients=(grad_honests + [grad_attack] * f_real), f=f_decl, bucket_size=bucket_size, gar_second=gar_second, pivot=pivot,
                sigma_filterL2=sigma_filterL2, device=device)
